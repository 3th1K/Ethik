...
internal class ApiErrorConfigServiceInitializer : IHostedService
{
    ...
    public ApiErrorConfigServiceInitializer(ApiErrorConfigService apiErrorConfigService, ILogger logger)
    {
        _apiErrorConfigService = apiErrorConfigService ?? throw new ArgumentNullException(nameof(apiErrorConfigService));
        _logger = logger.ForContext<ApiErrorConfigServiceInitializer>() ?? throw new ArgumentNullException(nameof(logger));
    }
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            _logger.Information("ApiErrorConfigService has been initialized.");
        }
        catch (Exception ex)
        {
            _logger.Error(ex, "Failed to initialize ApiErrorConfigService.");
            throw;
        }
