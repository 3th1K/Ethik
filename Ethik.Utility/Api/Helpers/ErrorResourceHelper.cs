...
namespace Ethik.Utility.Api.Helpers;
...
public class ErrorResourceHelper
{
    ...
    public ApiError GetResourceInfo(string errorId)
    {
        try
        {
            var errValue = _resourceManager.GetString(errorId) ?? throw new ErrorResourceException($"Key not found with the name [{errorId}]");
